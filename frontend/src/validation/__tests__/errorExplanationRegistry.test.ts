...
describe("errorExplanationRegistry", () => {
  ...
  describe("FHIR_INVALID_PRIMITIVE", () => {
    const handler = errorExplanationRegistry.FHIR_INVALID_PRIMITIVE;

    it("renders correct description with canonical schema", () => {
      const result = handler({
        actual: "1960-05-15x",
        expectedType: "date",
      ...
      expect(result.title).toBe("Invalid date value");
      expect(result.description).toBe("Cannot parse '1960-05-15x' as date");
    });

    it("renders correct description without reason", () => {
      const result = handler({
        actual: "invalid",
        expectedType: "boolean",
      });

      expect(result.title).toBe("Invalid boolean value");
      expect(result.description).toContain("'invalid'");
      expect(result.description).toContain("boolean");
    });

    it("handles missing details with fallback", () => {
      const result = handler(undefined);
      expect(result.title).toBe("Invalid primitive value");
      expect(result.description).toContain("cannot be parsed");
    });

    it("handles malformed details with fallback", () => {
      const result = handler("not an object");
      expect(result.title).toBe("Invalid primitive value");
      expect(result.description).toContain("FHIR primitive type");
    });
  });

  describe("FHIR_ARRAY_EXPECTED", () => {
    const handler = errorExplanationRegistry.FHIR_ARRAY_EXPECTED;
    ...
    it("renders correct description with canonical schema", () => {
      ...
      expect(result.title).toBe("Expected array");
      expect(result.description).toBe("This field expects an array of values, but received object.");
    });

    it("handles different actualType values", () => {
      const result = handler({
        expectedType: "array",
        actualType: "string",
      });
      ...
      expect(result.title).toBe("Expected array");
      expect(result.description).toContain("received string");
    });

    it("handles missing details with fallback", () => {
      const result = handler(undefined);
      expect(result.title).toBe("Expected array");
      expect(result.description).toContain("different type");
    });
